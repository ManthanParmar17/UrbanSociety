...
async function loadStats(memberId) {
    try {
        const [bills, complaints, events, members] = await Promise.all([
            fetch(`${API}/get_member_bills?member_id=${memberId}`).then(r => r.json()).catch(() => []),
            fetch(`${API}/get_member_complaints?member_id=${memberId}`).then(r => r.json()).catch(() => []),
            fetch(`${API}/get_events`).then(r => r.json()).catch(() => []),
            fetch(`${API}/get_members`).then(r => r.json()).catch(() => []),
        ]);

        const pendingBills = Array.isArray(bills) ? bills.filter(b => b.status !== "paid").length : 0;
        const openComplaints = Array.isArray(complaints) ? complaints.filter(c => c.status !== "resolved").length : 0;
...
async function loadNotices() {
    ...
    try {
        const data = await fetch(`${API}/get_notices`).then(r => r.json());
...
async function loadEvents() {
    ...
    try {
        const data = await fetch(`${API}/get_events`).then(r => r.json());
...
async function loadBills(memberId) {
    ...
    try {
        const bills = await fetch(`${API}/get_member_bills?member_id=${memberId}`).then(r => r.json());
        ...
        listEl.innerHTML = billArr
            .map(b => `<li><span class="pill ${b.status === "paid" ? "green" : "amber"}">${b.status || "pending"}</span> ₹${b.amount} · ${b.description || "Bill"}</li>`)
            .join("");
    } catch (e) {
